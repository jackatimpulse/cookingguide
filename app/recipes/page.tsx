...
export default function RecipesPage() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-6 py-12">
      ...
      <h1 className="font-heading text-3xl md:text-4xl font-light mb-3 text-[#000000]">Recipes</h1>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
        {rest.map((r) => (
          <Link key={r.slug} href={`/recipes/${r.slug}`} className="group bg-white border border-[#000000]/10 hover:border-[#205C49]/30 hover:shadow-md transition-all rounded-[10px] overflow-hidden">
            ...
            <div className="p-5">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-[#000000]/20 text-xs">·</span>
                <span className="text-[0.55rem] tracking-widest uppercase text-[#000000]/30">{r.prepTime + r.cookTime}m</span>
              </div>
              <h3 className="font-heading text-base font-light text-[#000000] group-hover:text-[#205C49] transition-colors">{r.title}</h3>
              <p className="text-xs text-[#000000]/40 mt-1 leading-relaxed line-clamp-2">{r.subtitle}</p>
            </div>
          </Link>
        ))}
      </div>
